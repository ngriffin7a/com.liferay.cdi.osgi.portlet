...
public class MethodSignature {
	...
	public boolean isMatch(Method method) {
		...
		if (returnType.equals(Void.TYPE) && isAssignableFrom(parameterTypes)) {
			...
			_variant1 && returnType.equals(Void.TYPE) &&
			(parameterTypes.length == 0)) {
			return true;
		}
		else if (
			_variant2 && returnType.equals(String.class) &&
			(parameterTypes.length == 0)) {
			return true;
		}

		_log.error(
			"Method {}.{} does not have a valid signature for @{}",
			method.getDeclaringClass()
				.getName(), method.getName(), _annotation.getSimpleName());
