...
public class BeanAppAnnotationImpl extends BeanAppBase {
	...
	public BeanAppAnnotationImpl(PortletApplication portletApplication) {
		...
		if (portletApplication == null) {
			...
			Arrays.stream(portletApplication.events()).map(
				eventDefinition ->
					EventDefinitionFactory.create(eventDefinition)).forEach(
				eventDefinition -> _eventDefinitions.add(eventDefinition));
			_publicRenderParamMap = new HashMap<>();
			Arrays.stream(portletApplication.publicParams()).map(
				prp -> PublicRenderParamFactory.create(prp)).forEach(
